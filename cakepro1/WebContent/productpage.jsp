	...
	<table>
		<tr>
			<c:forEach items="${list1 }" var="product" begin="${2*page.pageNO-2 }" end="${page.pageNO*2-1 }">
		        ${product.id}--${product.name }--<a>购买</a>--<a>品尝</a><br>
	         </c:forEach>
			...
			当前第${page.getPageNO() }页<br>
				...
				<c:choose>
					<c:when test="${page.pageNO!=page.getTotalPages() }">
						<a href="/cakepro1/user/showpage?pageNO=${page.getNextPageNo()}"><input type="button" name="previousPage"
						 value="下一页"></a>
					</c:when>
					<c:otherwise>
